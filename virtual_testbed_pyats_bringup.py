...
class virtual_harness():
  ...
  #     k=['/auto/nsstg-tools/bin/cot', 'edit-hardware', 'aa', '-o', 'bb', '-c', '2', '-m', '8GB', '-n', '10', '--nic-types' ,'virtio']
  #     for i in range (0, len(k)):
  #       if k[i]=="aa":
  #         k[i]=image
  #       if k[i]=="bb":
  ...
  #     print(k)
  #     p = subprocess.Popen(k,stdout=subprocess.PIPE,stderr=subprocess.PIPE)

  #     output, errors = p.communicate()
  #     if str(output)=="b''":
  #       print(banner("image %s with 10 NICS built" %image))
  #       src="/auto/iwan3x/iwan2x/ng_pfr/athena/"+new_name
  #       if image == hubImage:
  #         imageDict['hub'] = targetDST
  #       if image == transitHubImage:
  #         imageDict['transit'] = targetDST
...
#   qiangwa modi
  ...
  def generate_run_job(scr, testbed_name, job_type, topology_name, unique_id,job_params=''):
    global rprt
    ...
    os.chdir(drp)
    print("job_file_location: %s/%s"%(drp,job_name))
    make_job.job_make(job_name, scr, yaml_path, 'csr_image', job_type,job_params=job_params)
    print(banner("Sleeping for 6 minutes for testbed to come up...."))
    time.sleep(300)
    ...
    ret_code = os.system("easypy %s -xunit -xunit_dir /storage/users/dich/report/PI_regression/%s -mailto %s -archive_dir %s" %(job_name,job_type,mail_recipient, drp_tmp))
  ...
  if isPhysical=="False":
    ...
    for testbed in fnl['test']:
      ...
      for script in fnl['test'][testbed]['scripts']:
        ...
        rp=virtual_harness.generate_run_job(fnl['test'][testbed]['scripts'][script], testbed, job_type, topology_name, unique_id,job_params)
        print(rp)
        es=ds
