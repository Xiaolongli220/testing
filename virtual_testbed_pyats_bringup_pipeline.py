...
import make_job_pipeline as make_job
# import make_job
import start_dev
import housekeeping
...
#   qiangwa modi
    ...
    op=testbed.devices[server_ip].execute("vmcloud netdelete -t %s" %testbed_name, timeout=120)
    print(op)
    time.sleep(30)
  ...
  def generate_run_job(scr, testbed_name, job_type, topology_name, unique_id):
    ...
    ret_code = os.system("easypy -mailto %s -archive_dir %s %s -xunit -xunit_dir /storage/users/dich/report/PI_regression/" %(mail_recipient, drp_tmp, job_name))
