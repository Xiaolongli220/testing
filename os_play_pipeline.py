...
JOBS_SUPPORTED = ["xsanity", "route", "pdp", "lb", "neg", "misc", "history", "b2b", "sanity", "spl", "hp", "pi25regression", "pi25sanity", "precommitsanity", "idc", "et", "teacat", "plr"]
JOBS_TCL_REGRESSION = ["xsanity", "route", "pdp", "lb", "neg", "misc", "history", "b2b"]
JOBS_PYATS_REGRESSION = ["spl", "hp", "et", "teacat", "plr"]
...
def execute_virtual_test_harness(job_type,
    ...
    if job_type == "csr":
        ...
        virtual_testbed_bringup = "virtual_testbed_tcl_bringup_pipeline.py"
    elif test_framework == "pyats":
        virtual_testbed_bringup = "virtual_testbed_pyats_bringup_pipeline.py"
    else:
...
#    if logs_dir:
    if logs_dir == '123':
        ...
        script_command = "python %s -s %s -u 'cisco' -p 'cisco' -m 'p4p2' -d '/storage/users/dich/tcl_script/PI_pfrv3/virl/%s' --isPhysical False -c '%s' --topology_name '%s' --job_type '%s' --pi25 '%s' -ui '%s' --deleteTopology '%s' -hub_image '%s' -transit_hub_image '%s' -branch1_image '%s' -branch2_image '%s'" \
        % (virtual_testbed_bringup,
           servername,
           topology_name,
           input_configuration,
